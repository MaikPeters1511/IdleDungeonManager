  ...
  template: `
    <div class="space-y-8 animate-in zoom-in-95 duration-500 pb-20">
      <header class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 class="text-3xl font-black text-white tracking-tight">Guild Headquarters Upgrades</h2>
          <p class="text-slate-400 mt-1">Invest your funds and essence into permanent enhancements.</p>
        </div>
        ...
        <div class="space-y-10">
          ...
          <div class="premium-card bg-gradient-to-br from-secondary/15 via-transparent to-transparent border-secondary/20 flex flex-col lg:flex-row justify-between items-center gap-8 !p-8">
            <div class="space-y-3 max-w-2xl text-center lg:text-left">
              <span class="px-3 py-1 bg-secondary/20 text-secondary border border-secondary/30 rounded-full text-[10px] font-bold uppercase tracking-wider">Ascension Portal</span>
              <h3 class="text-2xl font-black text-white">Reincarnation</h3>
              ...
              <p class="text-xs text-slate-500 font-semibold italic">Formula: 1 Essence per 100,000 lifetime Gold earned.</p>
            </div>
            
            <div class="flex flex-col items-center p-6 bg-slate-950/60 rounded-2xl border border-white/5 min-w-[240px] text-center">
              <p class="text-xs font-bold text-slate-500 uppercase tracking-widest">Pending Reward</p>
          ...
          <div class="space-y-6">
            <div class="flex items-center justify-between">
              <h3 class="text-2xl font-black text-white tracking-tight">Ancient Relics</h3>
              <div class="flex items-center gap-2 px-4 py-2 bg-secondary/10 rounded-xl border border-secondary/20">
                <span class="text-xs text-secondary font-bold uppercase tracking-wider">Available Essence:</span>
              ...
              @for (relic of relics(); track relic.id) {
                <div class="premium-card flex flex-col h-full group border border-secondary/10 !p-5">
                  <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">Level {{ relic.level }}</span>
                  </div>

                  <h3 class="text-lg font-bold text-white mb-1">{{ relic.name }}</h3>
                  <p class="text-xs text-slate-500 flex-1 leading-relaxed">{{ relic.description }}</p>

                  <div class="mt-5 pt-4 border-t border-white/5 space-y-3">
                    <div class="flex justify-between items-center">
                      <span class="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Active Bonus</span>
...
export class UpgradesComponent {
  ...
  public prestige() {
    if (confirm('Are you ready to reincarnate? This resets your Gold, Guild Upgrades, Hero Levels, and locks your Dungeons, but gives you Essence to purchase Relics. This cannot be undone.')) {
